...
#ifndef RPMPROGRESS_H
#define RPMPROGRESS_H

#include "window.h"
#include "config.h"

class rpmprogress : public window
{
 public:
   rpmprogress( char *, unsigned long, int, int );

   void Next( char *, unsigned long );
   void Update( unsigned long );
   void SetTitle( char * );
   void IncTotal( unsigned long );
   
 private:
   unsigned long size, current_size;

   unsigned long total_size, total_current_size;
   int total_blockcount;
};
